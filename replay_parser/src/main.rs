...
use wot_replay_parser::{
    self,
    event::{ToPacket, TargetableEvent, battle_event::BattleEvent, BattleInfo, EventPrinter},
};
fn main() {
    let file = std::fs::read("/home/dacite/Projects/wot-battle-results-parser/replay_parser/examples/20220306_0941_france-F108_Panhard_EBR_105_45_north_america.wotreplay").unwrap();
    // let file = std::fs::read("/home/dacite/Projects/wot-battle-results-parser/replay_parser/examples/20220302_2332_germany-G42_Maus_45_north_america.wotreplay").unwrap();
    // let file = std::fs::read("/home/dacite/Projects/wot-battle-results-parser/replay_parser/examples/example.wotreplay").unwrap();
    // let file = std::fs::read("/home/dacite/Projects/wot-battle-results-parser/replay_parser/examples/20160603_2346_germany-G120_M41_90_GrandFinal_11_murovanka.wotreplay").unwrap();
    let result = wot_replay_parser::parse(&file).unwrap();
    std::fs::write("result.json", result.0[1].to_string().as_bytes()).unwrap();

    // let timer = Instant::now();
    let stream = wot_replay_parser::packet_stream::PacketStream::new(&result.1);
    let events = wot_replay_parser::event::event_stream::EventStream::new(stream);
    ...
    for i in results {
        ...
        // println!("{} {:+?}", get_replay_time(start_time as f64, i.get_as_packet().get_time() as f64), i.clone().get_as_packet(),);
    }
...
fn get_player_list(json: serde_json::Value) -> HashMap<u32, String> {
    ...
    for i in vehicles.into_iter() {
        ...
        player_list.insert(avatar_id, [name.to_string(), ", ".to_string(), tank.to_string()].concat());
    }
    player_list
}
