...
impl<'pkt> PacketParser<'pkt> for GameVersion<'pkt> {
    fn parse(packet: &'pkt Packet) -> Result<BattleEvent<'pkt>> {
        ...
        let (remaining, _) = take::<_, _, Error>(4 as usize)(packet.get_payload())?;
...
fn match_digit(s: &[u8]) -> Result<(&[u8], &str)> {
    let (remaining, digit) = take_while::<_, _, crate::Error>(is_digit)(s)?;
    let (remaining, _) = take_till::<_, _, Error>(is_digit)(remaining)?;

    let digit = std::str::from_utf8(digit).map_err(|err| Error::Other(err.to_string()))?;
    Ok((remaining, digit))
}

impl AsPacket for GameVersion<'_> {
...
mod tests {
    ...
    fn parses_correct_game_version_given_packet_data() {
        ...
        let game_version = crate::packet_parser::parse(&game_version_packet);
