...
///     - `00000000` For packet `0x08`, this is known as the `messageID` inside BigWorld (the game engine WoT for stuff
///       that does not include graphics). For us, it tell us which method of the entity is being called.
///     In this particular case, the `messageID` is 0 and refers to `showShooting()` method on the entity (the entity is
///     `Vehicle` for this ex.)
///     - `00000002` tells us how many bytes of information is left. In this case this is `2` bytes
///     - `0001` is the interesting part: the input for serde. In this particular case it is simply two values of u8s
///
/// ```
mod serde_packet;
